b3e.editor.ExportManager = function (editor) {
  ...
  this.projectToDataByTree = function () {
    var project = editor.project.get();
    if (!project) return;
  ...
  this.folderToData = function () {
    var project = editor.project.get();
    if (!project) return;

    var tree = project.trees.getSelected();
    var folder = project.folders.getSelected();
    console.log("folderToData:", folder)
    var data = {
      version: b3e.VERSION,
      scope: 'project',
      selectedTree: (tree ? tree._id : null),
    ...
    project.trees.each(function (tree) {
      var d = this.treeToData(tree, true);
      d.id = tree._id;
      if (folder.name === d.parent) {
        data.trees.push(d);
        console.log("folderToData.trees:", d)
